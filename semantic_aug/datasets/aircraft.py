...
DEFAULT_ROOT_DIR = os.environ.get("AIRCRAFT_ROOT", "datasets/aircraft/fgvc-aircraft-2013b/data")
DEFAULT_IMAGE_DIR = os.environ.get(
    "AIRCRAFT_IMAGE_DIR", os.path.join(DEFAULT_ROOT_DIR, "images"))
...
class AircraftDataset(FewShotDataset):
    ...
    def __init__(self, *args, split: str = "train", seed: int = 0, 
        ...
        split_dir = {"train": 'images_variant_trainval.txt', "val": 'images_variant_test.txt'}[split]

        class_to_images = defaultdict(list)
        with open(os.path.join(root_dir, split_dir)) as f:
            for line in f:
                image_id, class_name = split_values(line)
                class_to_images[class_name].append(os.path.join(image_dir, image_id + ".jpg"))
