...
from diffusers_.utils import logging
...
class SynGuidance2(GenerativeAugmentation):
    ...
    def __init__(self, embed_path: str,
        ...
        super(SynGuidance2, self).__init__()

        if SynGuidance2.pipe is None:
            ...
            SynGuidance2.pipe = PipelineClass.from_pretrained(model_path, 
                                                variant="fp16", 
        ...
        self.top_k = top_k
        self.do_similar_guidance = do_similar_guidance
        self.do_mean_guidance=do_mean_guidance
    ...
    def forward(self, image: Image.Image, label: int, 
        ...
        # class_prompt = [self.prompt.format(name=self.format_name(prmpt)) for prmpt in metadata.get("other_labels", [])]
        class_prompt = self.prompt.format(name=self.format_name("class_mean"))
        ...
        if self.strength < 1.0:  # use image as guidance
            kwargs = dict(
                ...
                do_mean_guidance=self.do_mean_guidance,
                equation=self.equation,
                # top_k=self.top_k,
                do_similar_guidance=self.do_similar_guidance,
