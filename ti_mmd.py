...
def parse_args():
    ...
    parser.add_argument("--dataset", type=str, default="coco", 
                        choices=["aircraft", "dtd", "coco", "pascal", "flowers", "pets", "cub"])
...
def main(args):
    ...
    trainable_params = 0
    for name, param in chain(text_encoder.named_parameters(), vae.named_parameters(), unet.named_parameters()):
...
if __name__ == "__main__":
    ...
    for seed, examples_per_class in options.tolist():
        # seed += 5
        if args.num_way is None:
            extract_folder = "extracted"
            findtune_folder = f"fine-tuned-mmd{args.dist_match}" 
        else:
            extract_folder = f"extracted-{args.num_way}"
            findtune_folder = f"fine-tuned-mmd{args.dist_match}-{args.num_way}"

        os.makedirs(os.path.join(output_dir, extract_folder), exist_ok=True)
        ...
        for idx in range(len(train_dataset)):
            ...
            path = os.path.join(output_dir, extract_folder, path, name, f"{idx}.png")
        ...
        for i, class_name in enumerate(train_dataset.class_names):
            ...
            args.placeholder_token = f"<class{i}>"
            if args.dataset == "dtd":
                args.initializer_token = "texture"
            if args.initializer_token is None:
                args.initializer_token = "the"

            args.train_data_dir = os.path.join(
                output_dir, extract_folder, dirname)
            args.output_dir = os.path.join(
                output_dir, findtune_folder, dirname)
