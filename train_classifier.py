...
from semantic_aug.datasets.aircraft import AircraftDataset
from semantic_aug.datasets.dtd import DTDDataset
...
from semantic_aug.augmentations.active_guidance import ActiveGuidance
from semantic_aug.augmentations.syn_guidance2 import SynGuidance2
from semantic_aug.augmentations.textual_inversion_per import TextualInversionPer
...
CLASSIFIER_PATH = "{dataset}-baselines/best_model_{seed}_{examples_per_class}.pt"

DEFAULT_EMBED_PATH = "{dataset}-tokens/{dataset}-{seed}-{examples_per_class}.pt"
DEFAULT_EMBED_PATH_MMD = "{dataset}-tokens-mmd{dist_match}/{dataset}-{seed}-{examples_per_class}.pt"
# DEFAULT_EMBED_PATH = "{dataset}-tokens-10way/{dataset}-{seed}-{examples_per_class}.pt"
# PER_IMAGE_EMBED_PATH = "{dataset}-tokens-10way-per-image/{dataset}-{seed}-{examples_per_class}.pt"
# DISC_EMBED_PATH = "{dataset}-tokens-tuned-w{weight}/{dataset}-{seed}-{examples_per_class}.pt"
# DISC_EMBED_PATH_1k = "{dataset}-tokens-tuned-w{weight}-1.0k/{dataset}-{seed}-{examples_per_class}.pt"
# DISC_EMBED_PATH_2k = "{dataset}-tokens-tuned-w{weight}-2.0k/{dataset}-{seed}-{examples_per_class}.pt"


# DISC_EMBED_PATH = "{dataset}-tokens-tuned-v2-w{weight}/{dataset}-{seed}-{examples_per_class}.pt"
...
AUGMENTATIONS = {
    ...
    "active-guidance": ActiveGuidance,
    "syn-guidance2": SynGuidance2,
    "textual-inversion-per": TextualInversionPer,
}
...
def run_experiment(
                   ...
                   equation: int = 3,
                   top_k: int = 0,
                   do_similar_guidance: bool = False,
                   do_mean_guidance: bool = False,
                   dist_match: float = 0.003,
                   weight: float = None,
                   mask: List[bool] = None,
    ...
    print(aug)
    if aug is not None:
        if "active-guidance" in aug:
            classifier_path = CLASSIFIER_PATH.format(dataset=dataset, seed=seed, examples_per_class=examples_per_class)
        else:
        ...
        aug = COMPOSERS[compose]([
            ...
            AUGMENTATIONS[aug](
                ...
                equation=equation,
                top_k=top_k,
                do_similar_guidance=do_similar_guidance,
                do_mean_guidance=do_mean_guidance,
                mask=mask, 
                inverted=inverted,
    ...
    for epoch in trange(num_epochs, desc="Training Classifier"):
        ...
        if aug is None:
            if best_acc < validation_accuracy.mean():
                best_acc = validation_accuracy.mean()
                if not os.path.exists(f"{dataset}-baselines"):
                    os.makedirs(f"{dataset}-baselines", exist_ok=True)
                torch.save(model.state_dict(), f"{dataset}-baselines/best_model_{seed}_{examples_per_class}.pt")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-way", type=int, default=None)
    ...
    parser.add_argument("--dataset", type=str, default="pets", 
                        choices=["aircraft", "dtd", "coco", "pascal", "flowers", "caltech", "pets", "cub"])
    
    parser.add_argument("--aug", nargs="+", type=str, default=None, 
                        choices=["real-guidance", "textual-inversion", "textual-inversion-per",
                                 "multi-token-inversion", "active-guidance", "syn-guidance2"])
    ...
    parser.add_argument("--equation", type=int, default=3)
    parser.add_argument("--top-k", type=int, default=0)
    parser.add_argument("--do-similar-guidance", action="store_true")
    parser.add_argument("--do-mean-guidance", action="store_true")
    ...
    parser.add_argument("--dist_match", type=float, default=0.003)
    ...
    for seed, examples_per_class in options.tolist():
        ...
        hyperparameters = dict(
            ...
            top_k=args.top_k,
            weight=args.weight,
            do_similar_guidance=args.do_similar_guidance,
            do_mean_guidance=args.do_mean_guidance,
        ...
        if args.dist_match > 0:
                args.embed_path = DEFAULT_EMBED_PATH_MMD
        else:
            args.embed_path = DEFAULT_EMBED_PATH
        ...
        current_trial = run_experiment(
            synthetic_dir=synthetic_dir, 
            embed_path=embed_path, 
            **hyperparameters)

        pd.DataFrame.from_records(current_trial).to_csv(path)
